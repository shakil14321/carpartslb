...
<meta property="og:image" content="{{ $carPart->feature_image ? asset('public/images/parts/feature/' . $carPart->feature_image) : '' }}">
...
<meta name="twitter:image" content="{{ $carPart->feature_image ? asset('public/images/parts/feature/' . $carPart->feature_image) : '' }}">
                                ...
                                <div class="product__media--preview__items">
                                    <a class="product__media--preview__items--link glightbox"
                                        ...
                                        <img class="product__media--preview__items--img"
                                            src="{{ asset('public/images/parts/gallery/' . $img) }}" alt="product-gallery-img">
                                    </a>
                                    <div class="product__media--view__icon">
                                        <a class="product__media--view__icon--link glightbox"
                                            href="{{ asset('public/images/parts/gallery/' . $img) }}"
                    ...
                    <div class="single__product--nav swiper">
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide">
                                <div class="product__media--nav__items">
                                    <img class="product__media--nav__items--img"
                                        src="{{ asset('public/images/parts/gallery/' . $img) }}" alt="product-nav-img">
                                </div>
                            </div>
                            @endforeach
                        </div>
                ...
                <div class="product__details--info">
                    <form action="#">
                        ...
                        <div class="product__details--info__price mb-12">
                            <span class="current__price">{{ $carPart->sale_price ? '$'.$carPart->sale_price : ''
                                }}</span>
                            <span class="old__price">{{ $carPart->original_price ? '$'.$carPart->original_price : ''
                                }}</span>
                        </div>
                        <ul class="rating product__card--rating mb-15 d-flex">
                            ...
                            <li> @if($productReviews->count() > 0)
                                    <span class="rating__review--text">{{ $productReviews->count() . ' Reviews' }}</span>
                                @else
                        ...
                        <div class="product__variant">
                            ...
                            <div class="product__variant--list mb-15">
                                <div class="product__details--info__meta">
                                    ...
                                    <p class="product__details--info__meta--list"><strong>Category:</strong> <span>{{
                                            $carPart->carPartType->title ?? 'None' }}</span> </p>
                                            
                                    <p class="product__details--info__meta--list"><strong>Part#:</strong> <span>{{ ucwords($carPart->part_number ?? '') }}</span> </p>        
                                    ...
                                    <div class="part_brnad_image_sec">
                                        <img 
                                          ...
                                          alt="{{ $carPart->carPartBrand->title ?? 'Brand Image' }}" style="width:120px; height:60px; border-radius:10px;" 
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="product__reviews">
                                <div class="product__reviews--header">
                                    ...
                                    <div class="reviews__ratting d-flex align-items-center">
                                        ...
                                        <span class="reviews__summary--caption">Based on {{ $productReviews->count() ?? '' }} reviews</span>
                                    </div>
                                        ...
                                        <div class="reviews__comment--content">
                                            <div class="reviews__comment--top d-flex justify-content-between">
                                                <div class="reviews__comment--top__left">
                                                    <h3 class="reviews__comment--content__title h4">{{ ucwords($productReview->username ?? '') }}</h3>
                                                    <ul class="rating d-flex">
                                                        @for($i=1; $i <= 5; $i++)
                                                            <li class="rating__list">
                                                                <span class="rating__icon">
                                                ...
                                                <span class="reviews__comment--content__date">{{ $productReview->updated_at->format('M d, Y') ?? '' }}</span>
                                            </div>
                                            <p class="reviews__comment--content__desc">{{ $productReview->review ?? '' }}</p>
                                        </div>
                                    </div>
                                    @if($productReview->reply)
                                        <div class="reviews__comment--list margin__left d-flex">
                                            ...
                                            <div class="reviews__comment--content">
                                                <div class="reviews__comment--top d-flex justify-content-between">
                                                    <div class="reviews__comment--top__left">
                                                        <h3 class="reviews__comment--content__title h4">{{ $productReview->reply_admin_name ?? '' }}</h3>
                                                    </div>
                                                </div>
                                                <p class="reviews__comment--content__desc">{{ $productReview->reply ?? '' }}</p>
                                            </div>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div id="writereview" class="reviews__comment--reply__area">
                                    <form action="{{ route('review.store') }}" method="POST">
                                        ...
                                        <div class="row">
                                            <input type="hidden" name="product_id" value="{{ $carPart->id ?? '' }}">
                                            <input type="hidden" name="product_title" value="{{ $carPart->title ?? '' }}">
                                            <input type="hidden" name="product_url" value="{{ url()->current() }}">
                                            <input type="hidden" name="user_image" value="{{ Auth::user()->user_image }}">
                                            <input type="hidden" name="rating" id="rating" value="0">
                                            
                                            <div class="col-12 mb-10">
                                                <textarea class="reviews__comment--reply__textarea"
                                                    placeholder="Your Comments...." name="review">{{ old('review') }}</textarea>
                                            </div>
                                            <div class="col-lg-6 col-md-6 mb-15">
                                                <label>
                                                    <input class="reviews__comment--reply__input"
                                                        placeholder="Your Name...." type="text" value="{{ Auth::user()->name ?? ''}}" readonly>
                                                </label>
                                            </div>
                                            <div class="col-lg-6 col-md-6 mb-15">
                                                <label>
                                                    <input class="reviews__comment--reply__input"
                                                        placeholder="Your Email...." type="email" value="{{ Auth::user()->email ?? '' }}" readonly>
                                                </label>
                                            </div>
                                        </div>
                                ...
                                @else
                                <h3 class="text-danger fw-bold">To share your valuable review, please log in <a href="{{ route('login.form') }}" class="text-decoration-underline">log in</a> first.</h3>
                                @endif
                            </div>
                            <script>
                ...
                <div class="swiper-slide">
                    <article class="product__card">
                        <div class="product__card--thumbnail">
                            <a class="product__card--thumbnail__link display-block" href="{{ route('product.view', $related->slug) }}">
                        ...
                        <div class="product__card--content">
                            <h3 class="product__card--title"><a href="{{ route('product.view', $related->slug) }}">{{
                                    $related->title ?? '' }}</a>
                            </h3>
                            ...
                            <div class="d-flex justify-content-between">
                                <div>
                                    <p class="oth_brand_title"><span class="oth_p">Brand:</span> <span class="rating__review--text">{{
                                            ucwords($carPart->carBrand->title ?? '') }}</span></p>
                            ...
                            <div class="product__card--price">
                                <span class="current__price">{{ $related->sale_price ? '$'.$related->sale_price :
                                    ''}}</span>
                                <span class="old__price"> {{ $related->original_price ? '$'.$related->original_price :
                                    ''}}</span>
                            </div>
